...
- The shooter of the previous pass can not be the receiver of the current pass, where the last pass does not depend on whether it is valid or not. This means that two robots can not pass back and forth without changing positions. You need *at least 3 robots*.
- A pass must be directed at least *10°* away from where the last pass came from, where the last pass does not depend on whether it is valid or not. 
